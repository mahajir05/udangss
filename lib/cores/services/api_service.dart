...
class ApiService {
  ...
  ApiService baseUrl(String baseUrl) {
    ...
    tokenDio.options.baseUrl = Environment.baseUrl();
    return this;
  }
  ...
  Future<Response> putList({
    ...
    try {
      ...
      response = await dio.put(
        apiPath,
        data: useFormData ? request?.map((e) => FormData.fromMap((e as Map<String, dynamic>))).toList() : request,
        cancelToken: cancelToken,
      );
      appPrint('Success [METHOD PUT] $apiPath');
      appLog('Success Response : ${response.data}');
  ...
  // }) {
  //   try {
  //     SecureStorageService().logout(AppNavigation.rootNavigatorKey.currentContext!);
  //   } catch (e) {
